...
-- Understanding overall delivery performance
...
-- Insight:
-- Deliveries are on average late, with delays reaching up to 30 minutes


-- 2. DELIVERY PERFORMANCE DISTRIBUTION
-- How deliveries are distributed across categories
...
-- Insight:
-- Nearly half of all deliveries are late (~49%)
...
-- Insight:
-- Distance is nearly identical across all categories indicating that its not a driver of delays
...
-- Insight:
-- Late deliveries remain consistently high throughout the day (47–51%)
-- Time of day is not a major driver
...
-- Insight:
-- All days fall within a very narrow range (~48–50% late)
-- No meaningful day level variation
...
-- Insight:
-- Ratings for late deliveries are stable across months
...
-- Insight:
-- Order size is almost identical across categories indicating that its not a driver of delays
...
-- Insight:
-- Some areas exceed 57–60% late deliveries vs ~ 49% overall
-- Area plays a key role


-- 9. AREA DELAY SEVERITY
-- Checking if delays are worse or just more frequent
...
-- Insight:
-- Delay duration is similar across areas (5.7–6.1 min)
-- Issue is mainly frequency, not severity
...
-- Insight:
-- Each partner ID is unique and can therefore not evaluate performance
-- Field behaves like an order identifier
...
-- Insight:
-- Late deliveries have significantly lower ratings (3.58 vs 4.50)
